...
**26 of 34 ADRs are Accepted.** 23 were ratified 2026-08-17 under [RATIFICATION-v0.1.0.md](RATIFICATION-v0.1.0.md); ADR-024 was ratified 2026-08-31 under [RATIFICATION-v0.5.0.md](RATIFICATION-v0.5.0.md), which also records why ADR-031 and ADR-033 were **not**. [RATIFICATION-v0.6.0.md](RATIFICATION-v0.6.0.md), ratified 2026-09-04, records the second departure, for v0.6.0. ADR-029 and ADR-031 were accepted **in reduced form** on 2026-09-26 under [RATIFICATION-v0.8.0.md](RATIFICATION-v0.8.0.md), the first release under ADR-035, which also records v0.7.0's missing record. ADR-020, ADR-030, ADR-032, ADR-033, ADR-034, ADR-035 and ADR-036 are held `Proposed`; ADR-014 is `Superseded` by the 023/024 split.

> **Runtime-dependent and still `Proposed`: ADR-034.** Derived by `conformance/skill.py` with [ADR-035](035-a-release-may-depend-on-a-proposed-adr-only-on-declared-and-expiring-terms.md) rule 1's scope: `engine/`, bound adapters, `tools/` (excluding measurement tools) and `.claude-plugin/`. The narrower scope used before 2026-09-26 read only `engine/` and bound adapters, which hid ADR-029 through eleven releases. Each release with a `Proposed` dependency has a ratification record the check verifies, and a dependency expires after two releases: ADR-034 reaches that limit with v0.8.0. Recording a dependency is not ratifying it (§68). Per §61, the project cannot reach `IMPLEMENTATION_READY` until the `RG-SIM-ONBOARDING-v0.1` simulation passes.
...
| [036](036-an-agent-that-reaches-authority-source-missing-discloses-rather-than-refuses.md) | An agent that reaches AUTHORITY_SOURCE_MISSING discloses rather than refuses | Agent surface | Split from ADR-031 when its evidenced engine rule was accepted and this half was not |
...
| 6. One viable adapter per required core role | ✅ **Met** — verified by `engine/completion.py GATE-6` → `STOP_COMPLETE`, not by assertion | All 16 adapters pass [Layer 1](../../conformance/layer1.py) **199/199**; [Layer 2](../../conformance/layer2.py) EQUIVALENT across 12 scenarios; criteria in `.repo-governor/acceptance/GATE-6.json` |
