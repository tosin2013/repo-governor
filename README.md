...
| | | |
| --- | --- | --- |
| **Implementation architecture** | **READY** | 15 adapters, a deterministic engine, 17 conformance suites. `RG-SIM-ONBOARDING-v0.1` passes; all 7 gate conditions ([§61](docs/reference/onboarding.md)) are engine-verified against live GitHub issues. |
...
```bash
git clone --branch v0.3.0 https://github.com/tosin2013/repo-governor /tmp/rg
...
```text
adapters/      15 provider adapters, subprocess protocol, any language
engine/        deterministic policy engine, Python stdlib only
conformance/   17 suites — the evidence behind every gate claim
schemas/       manifest v1 JSON Schema
docs/
  adrs/        30 architectural decisions (23 Accepted) + index + ratification review
...
| Suite | Asserts |
| --- | --- |
| `layer1.py` | 149 contract checks across 15 adapters |
